...
bool Config::_isFSempty() {
  ...
  char fullpath[64]; // Безпечний розмір для LittleFS
...
void Config::init() {
  ...
  // Для ESP32 параметр (true) дозволяє форматування за помилки монтування
  if (!LittleFS.begin(true)) {
    Serial.println("##[ERROR]#\tLittleFS Mount Failed");
    return;
  }
  BOOTLOG("LittleFS mounted");
  // Примусово створюємо необхідні папки для класичного ESP32
  if (!LittleFS.exists("/www")) {
//    LittleFS.mkdir("/www");
    Serial.println("[LittleFS] Created /www directory");
  }
  if (!LittleFS.exists("/data")) {
//    LittleFS.mkdir("/data");
    Serial.println("[LittleFS] Created /data directory");
  }
  // Перевірка на порожнечу (метод _isFSempty теж повинен використовувати LittleFS всередині)
   emptyFS = _isFSempty();
  if(emptyFS) BOOTLOG("LittleFS is empty!");
  ssidsCount = 0;
  // Робота з вказівниками на файлову систему
  #ifdef USE_SD
  // Якщо режим SD – використовуємо sdman, інакше – адреса об'єкта LittleFS
...
void Config::changeMode(int newmode){
  ...
  // Оновлюємо вибір файлової системи: якщо не картка SD, то використовуємо LittleFS
  _SDplaylistFS = getMode()==PM_SDCARD?&sdman:(true?&LittleFS:_SDplaylistFS);
  if(getMode()==PM_SDCARD){
...
bool Config::littlefsCleanup(){
  // Перевіряємо існування файлів перед видаленням (використовуємо LittleFS)
...
void Config::resetSystem(const char *val, uint8_t clientId){
  ...
  if (strcmp(val, "timezone") == 0) {
    ...
    saveValue(store.sntp2, "ua.pool.ntp.org", 35);
  ...
  if (strcmp(val, "weather") == 0) {
    ...
    saveValue(store.weatherlat, "50.7485", 10, false);
    saveValue(store.weatherlon, "31.0295", 10, false);
...
void Config::setDefaults() {
  ...
  strlcpy(store.sntp2,"ua.pool.ntp.org", 35);
  store.showweather=false;
  strlcpy(store.weatherlat,"50.7485", 10);
  strlcpy(store.weatherlon,"31.0295", 10);
...
void Config::indexPlaylist() {
  // Відкриваємо файл плейлиста для читання з LittleFS
  File playlist = LittleFS.open(PLAYLIST_PATH, "r");
  if (!playlist) {
    return;
  }
  int sOvol;
  // Створюємо файл індексу для запису в LittleFS
  File index = LittleFS.open(INDEX_PATH, "w");
  while (playlist.available()) {
    uint32_t pos = playlist.position();
	//Зчитуємо рядок і якщо він валідний (CSV), записуємо позицію в індекс
...
//  strncpy(_stationBuf, playlist.readStringUntil('\t').c_str(), sizeof(_stationBuf));
  strlcpy(_stationBuf, playlist.readStringUntil('\t').c_str(), sizeof(_stationBuf));
...
void Config::bootInfo() {
  ...
  BOOTLOG("chip:\t\tmodel: %s | rev: %d | id: %u | cores: %d | psram: %u", ESP.getChipModel(), ESP.getChipRevision(), chipId, ESP.getChipCores(), ESP.getPsramSize());
  BOOTLOG("display:\t%d", DSP_MODEL);
  if(VS1053_CS==255) {
    BOOTLOG("audio:\t\t%s (%d, %d, %d, %d)", "I2S", I2S_DOUT, I2S_BCLK, I2S_LRC, I2S_MCLK);
  }else{
  ...
  // ФІНАЛЬНИЙ ЗВІТ ПО ПАМ'ЯТІ ТА ЯДРАМ
  BOOTLOG("CPU Freq:\t%u MHz", (unsigned int)ESP.getCpuFreqMHz());
  BOOTLOG("PSRAM free:\t%u / %u bytes", (unsigned int)ESP.getFreePsram(), (unsigned int)ESP.getPsramSize());
  BOOTLOG("SRAM free:\t%u bytes", (unsigned int)ESP.getFreeHeap());
  BOOTLOG("Max alloc:\t%u bytes", (unsigned int)ESP.getMaxAllocHeap());
  BOOTLOG("CPU Cores:\t%d (Boot Core: %d)", (int)ESP.getChipCores(), (int)xPortGetCoreID());
  BOOTLOG("------------------------------------------------");
}
