...
void NetServer::processQueue(){
  ...
  if(xQueueReceive(nsQueue, &request, NS_QUEUE_TICKS)){
    ...
    switch (request.type) {
      ...
      case SDPOS:         sprintf (wsBuf, "{\"sdpos\": %u,\"sdend\": %u,\"sdtpos\": %u,\"sdtend\": %u}", 
                                  player.getFilePos(), 
                                  player.getFileSize(), 
                                  player.getAudioCurrentTime(), 
                                  player.getAudioFileDuration()); 
                                  break;
      case SDLEN:         sprintf (wsBuf, "{\"sdmin\": %u,\"sdmax\": %u}", player.sd_min, player.sd_max); break;
  ...
  if(request->url()=="/upload"){
    if (!index) {
      if(filename!="tempwifi.csv"){
        //player.sendCommand({PR_STOP, 0});  // розкоментувати якщо будуть перезавантаження
    ...
    if (!index) {
      player.sendCommand({PR_STOP, 0});
      String spath = "/www/";
	  // Якщо це плейлист або налаштування Wi-Fi, зберігаємо в папку /data/
      if(filename=="playlist.csv" || filename=="wifi.csv") spath = "/data/";
      request->_tempFile = LittleFS.open(spath + filename , "w");
    }
    if (len) {
      if (request->_tempFile) { // Додаємо перевірку, що файл відкритий
	  request->_tempFile.write(data, len);
	  }
    }
    if (final) {
  ...
  if(network.status == CONNECTED){
    ...
    if(paramsNr==1){
      ...
      if(cmd.exec(p->name().c_str(),p->value().c_str())) {
        ...
        if(p->name()=="clearlittlefs") { delay(100); ESP.restart(); }
